...
RegisterNetEvent("qb-drugs:client:getoxytruck")
AddEventHandler("qb-drugs:client:getoxytruck", function() 
if TriggerServerEvent('qb-drugs:server:payfortruck') then
end
end)
...
RegisterNetEvent("qb-drugs:Client:getoxylocation", function()
	...
	TriggerEvent("qb-drugs:client:getoxylocationroute")
    SetVehicleEngineOn(oxycar, true, true)
	 exports['qb-target']:AddTargetEntity(oxycar, {
    options = {
        {
            event = "qb-drugs:client:getfromtrunk",
...
RegisterNetEvent("qb-drugs:client:getoxylocationroute")
AddEventHandler("qb-drugs:client:getoxylocationroute", function() 
	Wait(100)
        ...
        options = {
            {
                ...
                label = "Talk To Buyer",
                icon = "fas fa-eye",
                event = "qb-drugs:client:giveoxybox"
            },
        }
     })
end)
...
RegisterNetEvent("qb-drugs:client:getfromtrunk")
AddEventHandler("qb-drugs:client:getfromtrunk", function() 
...
RegisterNetEvent("qb-drugs:client:giveoxybox")
AddEventHandler("qb-drugs:client:giveoxybox", function() 
	if carryPackage then
		QBCore.Functions.Progressbar("drink_something", "handing over oxy", 4000, false, true, {
			...
			TriggerServerEvent("qb-drugs:server:giveoxybox")
