							...
							options = {
								{
									...
									action = function()
										...
										drying = false
										TriggerServerEvent('qb-drugs:server:dryoutweed')
									end,
									canInteract = function()
										if Config.Joblock then
											if  QBCore.Functions.GetPlayerData().job.name == Config.weedjob then
...
options = {
		{
			...
			action = function()
						...
						if success then
							TriggerServerEvent("qb-drugs:server:makebutter")       
							ClearPedTasks(PlayerPedId())
							else
							ClearPedTasks(PlayerPedId())
							end
		...
		},
		{
			...
			action = function()
					...
					exports['ps-ui']:Circle(function(success)
					if success then
						TriggerServerEvent("qb-drugs:server:makebrownies")       
						ClearPedTasks(PlayerPedId())
					else
						ClearPedTasks(PlayerPedId())
					end
		...
		},
		{
			...
			action = function()
					...
					exports['ps-ui']:Circle(function(success)
					if success then
						TriggerServerEvent("qb-drugs:server:makecookies")       
						ClearPedTasks(PlayerPedId())
					else
						ClearPedTasks(PlayerPedId())
					end
		...
		},
		{
			...
			action = function()
					...
					exports['ps-ui']:Circle(function(success)
					if success then
						TriggerServerEvent("qb-drugs:server:makechocolate")       
						ClearPedTasks(PlayerPedId())
					else
						ClearPedTasks(PlayerPedId())
					end
		...
		},
		{
			...
			action = function()
					...
					exports['ps-ui']:Circle(function(success)
					if success then
						TriggerServerEvent("qb-drugs:server:makemuffin")
						ClearPedTasks(PlayerPedId())
					else
						ClearPedTasks(PlayerPedId())
					end
...
options = {
		{
			...
			action = function()
						...
						QBCore.Functions.Progressbar("drink_something", "Making Wax Oil", 4000, false, true, {
							...
							exports['ps-ui']:Circle(function(success)
						if success then
							TriggerServerEvent("qb-drugs:server:makeoil")       
							ClearPedTasks(PlayerPedId())
						else
...
RegisterNetEvent('qb-drugs:client:bluntwraps', function(args)
  lib.registerContext({
    id = 'bluntroll',
    title = 'Blunt Types',
    menu = 'bluntroll',
...
RegisterNetEvent("qb-drugs:client:rollanim", function()
	TriggerEvent('animations:client:EmoteCommandStart', {'uncuff'}) 
Wait(4000)
ClearPedTasks(PlayerPedId())
end)
...
RegisterNetEvent("qb-drugs:client:dodabs", function()
	TriggerEvent('animations:client:EmoteCommandStart', {'uncuff'}) 
AlienEffect()
end)

RegisterNetEvent("qb-drugs:client:edibles", function()
...
for k, v in pairs (Config.Weed.items) do 
	WeedShop[#WeedShop + 1] = {
		icon = "nui://"..Config.imagelink..QBCore.Shared.Items[v.name].image,
		 header = v.label,
		 title = v.label,
		 event = "qb-drugs:client:WeedShop",
		 args = {
			 item = v.name,
			 cost = v.price,
			 amount = v.amount,
...
RegisterNetEvent("qb-drugs:client:WeedShop", function(data)
	...
	if data.cost == "Free" then data.cost = 0 end
		TriggerServerEvent("qb-drugs:server:travellingmerchantox", dialog[2], dialog[1], data.item, data.cost )
end)

RegisterNetEvent('qb-drugs:client:smokeblunts', function(itemName)
