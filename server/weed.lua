...
--------------- events
RegisterServerEvent('qb-drugs:server:dryoutweed', function()
	local src = source
...
RegisterServerEvent('qb-drugs:server:makebutter', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:server:makebrownies', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:server:makecookies', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:server:makechocolate', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:server:makemuffin', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
  local recipe = {"cannabutter", "flour"}
  local butter = Player.Functions.GetItemByName("cannabutter")
...
RegisterServerEvent('qb-drugs:server:makeoil', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:server:rollblunt', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
  ...
  for k, v in pairs (recipe) do 
		if butane and butane.amount > 0 then
			if grind and grind.amount > 0 then
				if Player.Functions.RemoveItem(v, 1) then
					TriggerClientEvent("qb-drugs:client:rollanim", src)
					price = 1
				end
			end	
		end
...
RegisterServerEvent('qb-drugs:server:rollleanblunt', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:server:rolldextroblunt', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:server:rollchewyblunt', function()
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
...
local src = source
local Player = QBCore.Functions.GetPlayer(src)
	if TriggerClientEvent('qb-drugs:client:smokeblunts', source, item.name) then
...
local src = source
local Player = QBCore.Functions.GetPlayer(src)
	if TriggerClientEvent("qb-drugs:client:edibles", source, item.name) then
		Player.Functions.RemoveItem(item.name, 1)
	end
end)
end
...
if Player.Functions.GetItemByName("butanetorch") then 
	if Player.Functions.RemoveItem("shatter", 1) then
    	...
    	TriggerClientEvent("qb-drugs:client:dodabs", src)
	else
...
local src = source
local Player = QBCore.Functions.GetPlayer(src)
TriggerClientEvent('qb-drugs:client:bluntwraps', src)
end)
