...
RegisterServerEvent('qb-drugs:server:payfortruck', function()
	local src = source
	...
	if Player.Functions.RemoveMoney('cash', Config.TruckPrice ) or Player.Functions.RemoveMoney('bank', Config.TruckPrice ) then
		TriggerClientEvent("qb-drugs:Client:getoxylocation", src)
...
RegisterServerEvent('qb-drugs:server:giveoxybox', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	if chance <= Config.OxyRunDitchChance then 
		...
		TriggerClientEvent("qb-drugs:client:getoxylocationroute", src)
